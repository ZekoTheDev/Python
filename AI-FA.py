...
naam = "Zakaria"
klas = "V1D"
studentnummer = 1826711

# Hier word de modules gebruikt om te kijken of het gegeven getal 0 over laat. Zo niet is het False
def is_even(n):
    """
...
# Doormiddel van de .floor() functie word het kleinere gehele getal teruggegeven
def floor(real):
    """ Bepaal het grootste gehele getal (int), dat kleiner dan of gelijk is aan real (float). """
    userInput = int(math.floor(real))
    return userInput

# Doormiddel van de .ceil() functie word het grotere gehele getal teruggegeven
def ceil(real):
    """ Bepaal het kleinste gehele getal (int), groter dan of gelijk aan real (float). """
    userInput = int(math.ceil(real))
    return userInput

# Door middel van een for loop kunnen we de gegeven getal delen met behulp van de modules om te kijken of het een deler
# is. Is dit het geval dan word dit toegevoegd aan de 'divisors' list
def div(n):
    """
    ...
    divisors = []
    for getal in range(1, n // 2 + 1):
        if n % getal == 0:
            divisors.append(getal)
    divisors.append(n)
    return sorted(divisors)

# Er word gecontroleerd of het gegeven getal een priem getal is. Als het getal 1 is krijg je direct false terug.
# Daarna word er een loop gemaakt op basis van het gehele getal, en kunnen we het alleen delen door zichzelf met behulp
#
# van de modulus. Lukt dit, dan is het een priem getal en word 'True' gereturned.
...
# met behulp van de bovenstaande 'is_prime()' functie controleren we of elk gegeven getal een priemgetal is.
# Als het een priem getal is word het direct aan de lijst 'primelist' toegevoegd
...
def primes(num):
    ...
    """
    primelist = []
    for getal in range(2, num):
        if is_prime(getal):
            primelist.append(getal)

    return sorted(primelist)

# Door de gegeven nummer te delen met een modulus in een While loop, kunnen we bepalen welke priemfactoren we hebben
# Die voegen we dan toe aan de 'factors' lijst
def primefactors(n):
    """
...
# We bepalen de grootste gemene deler door de twee getallen te vergelijken en na de vergelijking de modules operator
# te gebruiken om te kijken welke van de twee de grootste gemene deler is
def gcd(a, b):
    """
...
# We bepalen de kleinste gemene veelvoud door de twee getallen te vergelijken en na de vergelijking de modules operator
# te gebruiken om te kijken welke van de twee de kleinste gemene veelvoud is
...
def lcm(a, b):
    ...
    while True:
        if groter % a == False and groter % b == False:
            lcm = groter
            break
        groter += 1
    return lcm

# Eerst hebben we de noemers bepaald door de gegeven tellers en noemers met elkaar te vermenigvuldigen
# Vervolgens kunnen we die met elkaar optellen om de derde noemer te krijgen.
# Dit delen we dan weer door elkaar om de nieuwe noemers te maken. En dat stoppen we in de gcd functie hierboven.
def add_frac(n1, d1, n2, d2):
    ...
    noemerDrie = noemerEen + noemerTwee
    delerDrie = delerEen

    gc = gcd(noemerDrie, delerDrie)

    newN3 = noemerDrie / gc
    newD3 = delerDrie / gc
    uitkomst = int(newN3), int(newD3)
